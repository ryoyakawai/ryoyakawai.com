...
export class BLEMIDIUtils {
  ...
  async startBleMIDIService(service, charUUID) {
    ...
    this.characteristic.output = {
      send: this.characteristic.writeValueWithoutResponse
    }
    this.characteristic.input = {
      onmidimessage: this.getMidiEventHandleCallback()
    }
    this.characteristic.addEventListener('characteristicvaluechanged', this.onMIDIEvent.bind(this));
  }
  onMIDIEvent(event) {
    ...
    event.detail = this.parseMIDIMessage(out.slice().splice(2));
